...
export function pickPlatformFromProviders(
  ...
  hints?: { originalLanguage?: string | null; preferred?: PlatformId },
): PlatformId {
...
export async function fetchItemPlatform(
  ...
  try {
    ...
    return pickPlatformFromProviders(data.results?.[region]?.flatrate, hints);
  } catch {
    return hints?.preferred ?? "netflix";
  }
}
